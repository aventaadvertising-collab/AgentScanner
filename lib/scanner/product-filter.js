...
const CONTENT_REJECT_PATTERNS = [
  ...
  /^(my experience|i built|i tried|i created|i made|i spent|i updated|i switched|i replaced)\b/i,
  /^(a guide to|the ultimate guide|beginner'?s guide|complete guide)/i,
  /\b(tutorial|walkthrough|step.by.step|hands.on)\b/i,
  /\b(post[ -]?mortem|lessons? learned|what went wrong)\b/i,
  /\b(a (?:deep )?dive into|breakdown of|anatomy of)\b/i,
...
export function validateProduct(discovery) {
  ...
  // 1. Product registry sources — always products, with extra HF filtering
  if (PRODUCT_SOURCES.has(source)) {
    ...
    // HuggingFace extra quality gate — filter zero-engagement personal uploads
    if (source === "huggingface") {
      const stars = discovery.stars || 0;
      const downloads = discovery.downloads || 0;
      const descLen = desc.length;

      // Reject items with no engagement AND no meaningful description
      if (stars === 0 && downloads === 0) {
        return { isProduct: false, reason: "hf_no_engagement", confidenceBoost: 0 };
      }

      // Reject items with only region/meta tags as description
      if (descLen < 30 && stars < 3 && downloads < 50) {
        return { isProduct: false, reason: "hf_low_quality_desc", confidenceBoost: 0 };
      }

      // Reject quantizations and personal fine-tunes (second safety net)
      if (stars < 20 && /\b(GGUF|GPTQ|AWQ|GGML|EXL2?|Q[2-8]_[KMS]|FineTuned?|Fine[_-]?Tuned?|SFT|DPO|LoRA|QLoRA|\d+[_-]?epochs?)\b/i.test(title)) {
        return { isProduct: false, reason: "hf_derivative_model", confidenceBoost: 0 };
      }
    }
  ...
  // 9. Twitter — social posts, must link to a product URL
  if (source === "twitter") {
    if (!url) {
      return { isProduct: false, reason: "twitter_no_url", confidenceBoost: 0 };
    }
    if (isBlogUrl(url)) {
      return { isProduct: false, reason: "twitter_blog_url", confidenceBoost: 0 };
    }
    for (const pattern of CONTENT_REJECT_PATTERNS) {
      if (pattern.test(title)) {
        return { isProduct: false, reason: `twitter_content:${pattern.source}`, confidenceBoost: 0 };
      }
    }
    const hasProductUrl = PRODUCT_URL_PATTERNS.some((p) => p.test(url));
    if (!hasProductUrl) {
      const hasProductSignal = PRODUCT_BOOST_KEYWORDS.some((kw) => fullText.includes(kw));
      if (!hasProductSignal) {
        return { isProduct: false, reason: "twitter_no_product_signal", confidenceBoost: 0 };
      }
    }
    const boost = calcProductBoost(fullText, discovery);
    return { isProduct: true, reason: "twitter_product_link", confidenceBoost: boost };
  }

  // 10. Funding — always pass through (pre-filtered by funding scanner)
  if (source === "funding") {
    for (const pattern of CONTENT_REJECT_PATTERNS) {
      if (pattern.test(title)) {
        return { isProduct: false, reason: `funding_content:${pattern.source}`, confidenceBoost: 0 };
      }
    }
    const boost = calcProductBoost(fullText, discovery);
    return { isProduct: true, reason: "funding_announcement", confidenceBoost: boost };
  }
