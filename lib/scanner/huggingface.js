...
const NOISE_NAME_PATTERNS = [
  ...
  /^Repository$/i, /^Space$/i, /^Model$/i, // Generic names
];

// Model re-uploads, quantizations, and personal fine-tunes — NOT original products
const FINETUNE_PATTERNS = [
  /\bGGUF\b/i, /\bGPTQ\b/i, /\bAWQ\b/i, /\bGGML\b/i, /\bEXL2?\b/i,
  /\bQ[2-8]_[KMS]/i,           // Quantization levels: Q4_K_M, Q5_K_S, etc.
  /\bFineTuned?\b/i, /\bFine[_-]?Tuned?\b/i,
  /\b\d+[_-]?epochs?\b/i,      // "5 epochs", "1_epoch"
  /\bSFT\b/,                    // Supervised fine-tuning
  /\bDPO\b/,                    // Direct preference optimization
  /\bLoRA\b/i, /\bQLoRA\b/i,   // Adapter fine-tunes
  /\bmerge[_-]?of\b/i,         // Model merges
  /\bABD\b.*\bINCLUDED\b/i,    // Spam pattern
];
...
function passesQualityGate(item, isSpace = false) {
  ...
  // 1b. Skip re-uploads, quantizations, and personal fine-tunes
  // These are derivative works, not original products
  // Exception: items with high engagement (>20 likes) may be notable quantizations
  if (likes < 20) {
    for (const pattern of FINETUNE_PATTERNS) {
      if (pattern.test(name)) return false;
    }
  }
