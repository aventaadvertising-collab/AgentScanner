...
export function categorize(name, description, topics, pipelineTag) {
  // Direct HF pipeline_tag mapping — trusted source of truth
  ...
  // With score of 1, category must be clearly dominant (no second candidate)
  if (bestScore >= 2) return bestCategory;
  if (bestScore === 1 && secondBestScore === 0) return bestCategory;

  // Ambiguous categorization — not enough signal
  return null;
}
