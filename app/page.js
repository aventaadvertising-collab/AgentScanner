  ...
  return (
    <div style={{ "--bg": "#0A0B10", "--s1": "#12141C", "--s2": "#1A1D28", "--b1": "rgba(0,255,170,.06)", "--b2": "rgba(0,255,170,.12)", "--b3": "rgba(0,255,170,.18)", "--t1": "#F2F2F7", "--t2": "rgba(242,242,247,.65)", "--t3": "rgba(242,242,247,.38)", "--g": "#00FFAA", "--gg": "rgba(0,255,170,.2)", "--gd": "rgba(0,255,170,.08)", "--em": "#00FFAA", "--m": "'SF Mono', 'JetBrains Mono', 'Fira Code', monospace", "--f": "-apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', sans-serif", minHeight: "100vh", background: "var(--bg)", color: "var(--t1)", fontFamily: "var(--f)" }}>
      ...
      <style suppressHydrationWarning>{`
        ...
        @keyframes text-glow { 0%,100% { text-shadow: 0 0 30px rgba(0,255,170,.2) } 50% { text-shadow: 0 0 60px rgba(0,255,170,.3) } }
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-hover { transition: all .25s cubic-bezier(.4,0,.2,1); border: 1px solid var(--b1); border-left: 2px solid transparent; position: relative; }
        .card-hover:hover { border-left-color: var(--g); background: rgba(0,255,170,.015) !important; box-shadow: 0 4px 24px rgba(0,0,0,.3); }
        .card-hover::after { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--g), transparent); opacity: 0; transition: opacity .25s; pointer-events: none; }
        .card-hover:hover::after { opacity: .5; }
        ...
        .cta-btn:hover { transform: translateY(-2px); box-shadow: 0 0 50px rgba(0,255,170,.35), 0 12px 40px rgba(0,0,0,.4); }
        .ghost-btn { transition: all .2s; }
        .ghost-btn:hover { background: rgba(255,255,255,.05) !important; border-color: rgba(0,255,170,.15) !important; }
      `}</style>

      {/* ─── Ambient Background ─── */}
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        <div style={{ position: "absolute", top: "-20%", left: "30%", width: "50vw", height: "50vw", borderRadius: "50%", background: "radial-gradient(circle, rgba(0,255,170,.05) 0%, transparent 70%)", filter: "blur(80px)" }} />
        <div style={{ position: "absolute", top: "40%", right: "5%", width: "35vw", height: "35vw", borderRadius: "50%", background: "radial-gradient(circle, rgba(167,139,250,.03) 0%, transparent 70%)", filter: "blur(60px)" }} />
        <div style={{ position: "absolute", bottom: "0%", left: "10%", width: "40vw", height: "40vw", borderRadius: "50%", background: "radial-gradient(circle, rgba(0,255,170,.02) 0%, transparent 70%)", filter: "blur(60px)" }} />
        <div style={{ position: "absolute", inset: 0, opacity: 0.025, backgroundImage: "radial-gradient(circle, rgba(0,255,170,.5) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
      </div>
      ...
      <nav style={{ padding: "0 48px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", zIndex: 10, borderBottom: "1px solid var(--b1)", background: "rgba(10,11,16,.75)", backdropFilter: "blur(24px) saturate(180%)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, position: "relative", overflow: "hidden", background: "linear-gradient(135deg, #00FFAA, #00CC88, #00AA77)", display: "flex", alignItems: "center", justifyContent: "center", animation: "glow-pulse 4s ease-in-out infinite" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="/dashboard" style={{ padding: "8px 20px", borderRadius: 6, background: "#00FFAA", color: "#0A0B10", fontSize: 13, fontWeight: 800, textDecoration: "none", letterSpacing: ".01em", fontFamily: "var(--m)" }}>Get Started</a>
        </div>
      </nav>
      ...
      <section style={{ padding: "120px 48px 80px", textAlign: "center", position: "relative", zIndex: 10, maxWidth: 950, margin: "0 auto" }}>
        {/* Live badge */}
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 18px", borderRadius: 4, background: "rgba(0,255,170,.04)", border: "1px solid rgba(0,255,170,.12)", marginBottom: 32, animation: "fi .5s ease both" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00FFAA", animation: "lp 2s ease-in-out infinite", boxShadow: "0 0 8px rgba(0,255,170,.5)" }} />
          <span style={{ fontSize: 11, fontWeight: 700, color: "#00FFAA", fontFamily: "var(--m)", letterSpacing: ".04em" }}>11 SOURCES SCANNING EVERY MINUTE</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", animation: "fi .6s ease .3s both" }}>
          <a href="/screener" className="cta-btn" style={{ padding: "14px 40px", borderRadius: 8, background: "#00FFAA", color: "#0A0B10", fontSize: 15, fontWeight: 800, textDecoration: "none", letterSpacing: ".01em", boxShadow: "0 0 30px rgba(0,255,170,.25), 0 4px 20px rgba(0,0,0,.3)", fontFamily: "var(--m)" }}>
            Open Screener
          </a>
          <a href="/dashboard" className="ghost-btn" style={{ padding: "14px 40px", borderRadius: 8, border: "1px solid var(--b2)", background: "transparent", color: "var(--t1)", fontSize: 15, fontWeight: 600, textDecoration: "none" }}>
            Explore Dashboard
          </a>
        </div>
      </section>
      ...
      <section style={{ padding: "0 48px 80px", position: "relative", zIndex: 10, maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, animation: "fi .5s ease .4s both" }}>
          {[
            { label: "Products Tracked", value: 181, suffix: "+", color: "#00FFAA" },
            { label: "Categories", value: 28, color: "#A78BFA" },
            { label: "Intelligence Sources", value: 11, color: "#38BDF8" },
            { label: "Scans Per Day", value: 15840, suffix: "+", color: "#FBBF24" },
          ].map((s, i) => (
            <div key={i} style={{ background: "var(--s1)", border: "1px solid var(--b1)", borderRadius: 8, padding: "20px 22px", position: "relative", overflow: "hidden", textAlign: "center" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: "rgba(255,255,255,.04)", overflow: "hidden", borderRadius: "0 0 8px 8px" }}>
      ...
      <section style={{ padding: "40px 48px 80px", position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {[
            { icon: "⊛", title: "11 Data Sources", desc: "GitHub, PyPI, HuggingFace, HN, npm, Reddit, Product Hunt, arXiv, GitHub Trending, Dev.to, Lobsters — all scanning every minute.", color: "#00FFAA" },
            { icon: "◈", title: "Real-Time Screener", desc: "Live feed of new AI products as they launch. Grade-based confidence scoring, category filters, feed and grid views.", color: "#A78BFA" },
            { icon: "⟁", title: "Deep Intelligence", desc: "Every AI startup mapped with GitHub stars, downloads, upvotes, author profiles, and source-specific metadata.", color: "#38BDF8" },
            { icon: "⊿", title: "Product Dashboard", desc: "155+ products tracked with metrics, funding data, team growth, revenue estimates, and competitive positioning.", color: "#FBBF24" },
            ...
            <div key={i} className="card-hover" style={{ padding: "28px 26px", borderRadius: 16, background: "var(--s1)", animation: `fi-scale .4s ease ${.6 + i * .06}s both`, cursor: "default" }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: `${f.color}10`, border: `1px solid ${f.color}25`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18 }}>
      ...
      <section style={{ padding: "60px 48px", position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ background: "var(--s1)", border: "1px solid var(--b1)", borderRadius: 16, padding: "48px 40px", position: "relative", overflow: "hidden", animation: "fi .5s ease .7s both" }}>
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(90deg, transparent, rgba(0,255,170,.2), rgba(167,139,250,.15), transparent)" }} />
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: "#00FFAA", marginBottom: 10, fontFamily: "var(--m)" }}>SCANNING 24/7</div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
            {[
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", borderRadius: 8, background: "rgba(255,255,255,.02)", border: "1px solid var(--b1)", animation: `fi .3s ease ${.8 + i * .04}s both` }}>
                <span style={{ fontSize: 14, color: "var(--g)", opacity: 0.7 }}>{s.icon}</span>
                <span style={{ fontSize: 12, fontWeight: 600, color: "var(--t2)" }}>{s.name}</span>
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#00FFAA", animation: "lp 2s ease-in-out infinite", animationDelay: `${i * 0.2}s` }} />
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: "60px 48px 80px", position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto", animation: "fi .5s ease .8s both" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: "#A78BFA", marginBottom: 10, fontFamily: "var(--m)" }}>COVERAGE</div>
          <h2 style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-.02em" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
          {["Code & Dev Tools", "AI Agents", "Agent Frameworks", "Foundation Models", "Video Generation", "Image Generation", "Crypto-AI", "Search & Research", "Audio & Voice", "Inference & Serving", "Voice Agents", "Browser Use", "Sales AI", "Healthcare AI", "Robotics", "3D & Spatial AI", "Data & Analytics", "Security AI", "Education AI", "Legal AI"].map((c, i) => (
            <span key={i} style={{ padding: "8px 18px", borderRadius: 6, background: "var(--s1)", border: "1px solid var(--b1)", fontSize: 12, fontWeight: 600, color: "var(--t2)", transition: "all .15s", cursor: "default", fontFamily: "var(--m)" }}>{c}</span>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "40px 48px 100px", textAlign: "center", position: "relative", zIndex: 10 }}>
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "56px 48px", borderRadius: 16, background: "var(--s1)", border: "1px solid var(--b1)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at top center, rgba(0,255,170,.04) 0%, transparent 60%)", pointerEvents: "none" }} />
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", position: "relative" }}>
            <a href="/screener" className="cta-btn" style={{ padding: "14px 36px", borderRadius: 8, background: "#00FFAA", color: "#0A0B10", fontSize: 15, fontWeight: 800, textDecoration: "none", boxShadow: "0 0 30px rgba(0,255,170,.25)", fontFamily: "var(--m)" }}>
              Open Screener
            </a>
            <a href="/dashboard" className="ghost-btn" style={{ padding: "14px 36px", borderRadius: 8, border: "1px solid var(--b2)", background: "transparent", color: "var(--t1)", fontSize: 15, fontWeight: 600, textDecoration: "none" }}>
              View Dashboard
            </a>
          </div>
        </div>
